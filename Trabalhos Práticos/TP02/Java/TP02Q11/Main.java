...
//        return musics;
//    }
    public static Musicas[] coutingSort(Musicas[] musics, int idEntrada){

        int[] array = new int[idEntrada];
        for (int i = 0; i < idEntrada; i++) {
            array[i] = musics[i].getPopularidade();

        }

        int kk =1;
        int[] countSort = new int[getMaior(musics) + 1];
        int[] ordenar = new int[idEntrada];

        for (int i = 0; i < countSort.length;countSort[i] = 0, i++);

        for (int i = 0; i < idEntrada; countSort[musics[i].getPopularidade()]++, i++);

        for (int i = 1; i < countSort.length; countSort[i] += countSort[i-1], i++);

        for (int i = idEntrada-1; i >=0; ordenar[countSort[musics[i].getPopularidade()]-1]= musics[i].getPopularidade(), countSort[musics[i].getPopularidade()]--, i--);

        for (int i = 0; i < idEntrada; array[i]= ordenar[i], i++);


        for (int i = 0; i < 100; i++) {
            for (int k = 0; k < 100; k++) {
                if (array[i] == musics[k].getPopularidade()){
                    musics[k].imprimir();
                    musics[k].setPopularidade(0);
                }
            }
        }
        return musics;
    }
    public static int getMaior(Musicas[] musicas) {
        int maior = musicas[0].getPopularidade();

        for (int i = 1; i < 99; i++) {
            if(maior < musicas[i].getPopularidade()){
                maior = musicas[i].getPopularidade();
            }
        }
        return maior;
    }
...
//                    musicsOut[i].imprimir();
    ...
    public static void main(String[] args) throws ParseException {
        ...
        Musicas[] musicSorted = coutingSort(music, idEntrada);
        int test = 0;
        do {
            entrada[test] = MyIO.readLine();
        }while (test++ != 5);
